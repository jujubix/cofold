...
int main(int argc, char *argv[]){
  ...
  */
  ...
  if(args_info.dist_alpha_given) {
	  distAlpha = args_info.dist_alpha;
  }
  if(args_info.dist_tau_given) {
	  distTau = args_info.dist_tau;
  }
  ...
  if (distAlpha < 0 || distTau < 0) {
	  fprintf(stderr, "Must provide value for alpha and tau\n");
	  exit(1);
  }
  /*
  #############################################
  ...
  */
  while(
    ...
    */
    ...
    min_en = energy_of_structure(rec_sequence, structure, 0); // Jeff: added this line so energy is recalculated without scaling
