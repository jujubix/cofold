...
/*
        ...
        Copyright 2010-2013 Jeff R. Proctor and Irmtraud M. Meyer
...
PRIVATE dml_node     *DMLi     = NULL; /* DMLi[j] holds MIN(fML[i,k]+fML[k+1,j])  */
PRIVATE dml_node     *DMLi1    = NULL; /*             MIN(fML[i+1,k]+fML[k+1,j])  */
PRIVATE dml_node     *DMLi2    = NULL; /*             MIN(fML[i+2,k]+fML[k+1,j])  */
...
enum state {
		HAIRPIN_LOOP,
		STACK_LOOP_BULGE,
		ML_DECOMP_CLOSING_BP,
		COAXIAL_STACK_CLOSING_BP,
		FREE_ENDS,
		DANGLING_NUCLEOTIDES,
		ML_DECOMP,
		COAXIAL_STACK,
		NO_STATE};
...
struct TraceNode {
	...
	int iright;
	int jright;
	enum structure_type type_left;
	enum structure_type type_right;
};

int hasRightChild(struct TraceNode* node) {
	switch(node->s) {
	case NO_STATE:
	case HAIRPIN_LOOP:
	case STACK_LOOP_BULGE:
	case FREE_ENDS:
	case DANGLING_NUCLEOTIDES:
		return 0;

	case ML_DECOMP_CLOSING_BP:
	case COAXIAL_STACK_CLOSING_BP:
	case ML_DECOMP:
	case COAXIAL_STACK:
		return 1;
	}
...
int hasLeftChild(struct TraceNode* node) {
	switch(node->s) {
	case NO_STATE:
	case HAIRPIN_LOOP:
		return 0;
...
PRIVATE struct TraceNode* traceC;
PRIVATE struct TraceNode* traceFML;

void minState(struct TraceNode* node, int new_energy, enum state new_state,
		int new_ileft, int new_jleft, enum structure_type new_type_left,
		int new_iright, int new_jright, enum structure_type new_type_right) {
	if (new_energy < node->energy) {
		node->energy = new_energy;
		node->ileft = new_ileft;
		node->iright = new_iright;
		node->jleft = new_jleft;
		node->jright = new_jright;
		node->s = new_state;
		node->type_left = new_type_left;
		node->type_right = new_type_right;
	}
}

struct TraceNode* getLeftChild(struct TraceNode* node) {
	if (hasLeftChild(node)) {
		if (node->type_left == CIJ) {
			return &(traceC[indx[node->jleft]+node->ileft]);
		}
		else if (node->type_left == FMLIJ) {
			return &(traceFML[indx[node->jleft]+node->ileft]);
		}
	}
	return NULL;
}
struct TraceNode* getRightChild(struct TraceNode* node) {
	if (hasRightChild(node)) {
		if (node->type_right == CIJ) {
			return &(traceC[indx[node->jright]+node->iright]);
		}
		else if (node->type_right == FMLIJ) {
			return &(traceFML[indx[node->jright]+node->iright]);
		}
	}
	return NULL;
}

// debugging function. Prints values of the children of a given node
void checkChildren(struct TraceNode* current, int i, int j) {
	return;
	struct TraceNode* left = getLeftChild(current);
	struct TraceNode* right = getRightChild(current);


	if (current->energy == 0) {
		fprintf(stderr, "node 0 energy at (%d, %d):\n", i, j);
		fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", current->energy, current->s, current->ileft, current->jleft, current->type_left,
						current->iright, current->jright, current->type_right);
		if (left != NULL) {
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", left->energy, left->s, left->ileft, left->jleft, left->type_left,
					left->iright, left->jright, left->type_right);
		}
		if (right != NULL) {
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", right->energy, right->s, right->ileft, right->jleft, right->type_left,
					right->iright, right->jright, right->type_right);
		}
	}

	if (left != NULL) {
		if (left->energy == 0) {
			fprintf(stderr, "left child 0 energy at (%d, %d):\n", i, j);
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", current->energy, current->s, current->ileft, current->jleft, current->type_left,
							current->iright, current->jright, current->type_right);
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", left->energy, left->s, left->ileft, left->jleft, left->type_left,
					left->iright, left->jright, left->type_right);
	...
	if (right != NULL) {
		if (right->energy == 0) {
			fprintf(stderr, "right child 0 energy at (%d, %d):\n", i, j);
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", current->energy, current->s, current->ileft, current->jleft, current->type_left,
							current->iright, current->jright, current->type_right);
			fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", right->energy, right->s, right->ileft, right->jleft, right->type_left,
					right->iright, right->jright, right->type_right);
...
void generateDotBracket(const char *seqString, char* dotBracket) {
	int length = strlen(seqString);
	//char* dotBracket = (char*) malloc(sizeof(char) * length+2);
	memset(dotBracket, '.', length);
	dotBracket[length] = '\0';
	struct TraceNode* root = &(traceFML[indx[length] + 1]);
	struct TraceNode* queue[200];
	int queuePointer = 1;
	queue[0] = root;

	while(queuePointer > 0) {
		queuePointer--;
		struct TraceNode* current = queue[queuePointer];
		struct TraceNode* left = getLeftChild(current);
		struct TraceNode* right = getRightChild(current);
		...
		//				current->iright, current->jright, current->type_right);
		if (left != NULL) {
			queue[queuePointer] = left;
			queuePointer++;
			if (current->type_left == CIJ) {
				dotBracket[current->ileft - 1] = '(';
				dotBracket[current->jleft - 1] = ')';
				// add base pair
			}
		}
		if (right != NULL) {
			queue[queuePointer] = right;
			queuePointer++;
			if (current->type_right == CIJ) {
				dotBracket[current->iright - 1] = '(';
				dotBracket[current->jright - 1] = ')';
				// add base pair
			}
		}
	}
...
void min_dml(dml_node* dml, int new_energy, int new_ileft, int new_jleft, int new_iright, int new_jright) {
	if (new_energy < dml->energy) {
		dml->energy = new_energy;
		dml->ileft = new_ileft;
		dml->jleft = new_jleft;
		dml->iright = new_iright;
		dml->jright = new_jright;
	}
}
...
PRIVATE void get_arrays(unsigned int size){
  ...
  traceC = (struct TraceNode *) space(sizeof(struct TraceNode) * ((size*(size+1))/2+2));
  traceFML = (struct TraceNode *) space(sizeof(struct TraceNode) * ((size*(size+1))/2+2));
  ...
  DMLi  = (dml_node *) space(sizeof(dml_node)*(size+1));
  DMLi1 = (dml_node *) space(sizeof(dml_node)*(size+1));
  DMLi2 = (dml_node *) space(sizeof(dml_node)*(size+1));
...
PUBLIC void free_arrays(void){
  ...
  if(c)         free(c);
  if(fML)       free(fML);
  if(traceC)	free(traceC);
  if(traceFML)	free(traceFML);
  if(f5)        free(f5);
  if(f53)       free(f53);
  if(cc)        free(cc);
  if(cc1)       free(cc1);
  ...
  if(P)         free(P);
  indx = c = fML = f5 = f53 = cc = cc1 = fM1 = fM2 = Fmi =  NULL;
  DMLi = DMLi1 = DMLi2 = NULL;
...
PUBLIC float fold_par(const char *string,
  ...
  generateDotBracket(string, structure);
  //if(circular){
...
PRIVATE int separationModifier(int distance, int energy) {
	//return energy;
	//double alpha = 0.75;
	//double tau = 800.0;

	double scale = exp(-((double)distance)/distTau);
	double scale_alpha = distAlpha * scale + 1.0 - distAlpha;

	// separation modifier verification:
	//fprintf(stderr, "%f\t%f\t%d\t%d\t%f\t%f\t%d\n", distAlpha, distTau, distance, energy, scale, scale_alpha, (int)(scale_alpha * energy));

	if (energy > 0) {
		return (2 - scale_alpha) * energy;
	}
	else {
		return scale_alpha * energy;
	}
}
...
PRIVATE int fill_arrays(const char *string) {
  ...
  for (j = 1; j<=length; j++)
    for (i=(j>TURN?(j-TURN):1); i<j; i++) {
      //c[indx[j]+i] = fML[indx[j]+i] = INF;
    	traceC[indx[j]+i].energy = INF;
    	traceFML[indx[j]+i].energy = INF;
  ...
  for (i = length-TURN-1; i >= 1; i--) { /* i,j in [1..length] */
    ...
    for (j = i+TURN+1; j <= length; j++) {
      ...
      struct TraceNode* cnode = &(traceC[ij]);
      cnode->energy = INF;

      struct TraceNode* fmlnode = &(traceFML[ij]);
      fmlnode->energy = INF;
      ...
      if (type) {   /* we have a pair */
		...
		//fprintf(stderr, "%d|%d|%d\t%d\t%d|%d\t%d\t%d\n", cnode->energy, cnode->s, cnode->ileft, cnode->jleft, cnode->type_left,
		//		cnode->iright, cnode->jright, cnode->type_right);
        if (no_close) {
        	//new_c = FORBIDDEN;
        	cnode->energy = FORBIDDEN;
        }
        else {
        	new_c = E_Hairpin(j-i-1, type, S1[i+1], S1[j-1], string+i-1, P);
        	new_c = separationModifier(j-i, new_c);
        	minState(cnode, new_c, HAIRPIN_LOOP, -1, -1, FMLIJ, -1, -1, FMLIJ);
        }
        ...
        for (p = i+1; p <= MIN2(j-2-TURN,i+MAXLOOP+1) ; p++) {
          ...
          for (q = minq; q < j; q++) {
            ...
            energy = separationModifier(j-i, energy);
            minState(cnode, energy+traceC[indx[q]+p].energy, STACK_LOOP_BULGE, p, q, CIJ, -1, -1, FMLIJ);
        ...
        if (!no_close) {
          int MLenergy;
          dml_node* dml = &(DMLi1[j - 1]);
          decomp = dml->energy;
          tt = rtype[type];
          switch(dangle_model){
            /* no dangles */
            case 0:   decomp += separationModifier(j-i, E_MLstem(tt, -1, -1, P) + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);
                      break;

            /* double dangles */
            case 2:   decomp += separationModifier(j-i, E_MLstem(tt, S1[j-1], S1[i+1], P) + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);
                      break;
            ...
            default:  decomp += separationModifier(j-i, E_MLstem(tt, -1, -1, P) + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);

            			dml =  &(DMLi2[j-1]);
            			decomp = dml->energy + separationModifier(j-i, E_MLstem(tt, -1, S1[i+1], P) + P->MLbase + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);
                        //decomp = MIN2(decomp, DMLi2[j-1] + separationModifier(j-i, E_MLstem(tt, -1, S1[i+1], P) + P->MLbase));

            			dml =  &(DMLi2[j-2]);
            			decomp = dml->energy + separationModifier(j-i, E_MLstem(tt, S1[j-1], S1[i+1], P) + 2*P->MLbase + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);
                        //decomp = MIN2(decomp, DMLi2[j-2] + separationModifier(j-i, E_MLstem(tt, S1[j-1], S1[i+1], P) + 2*P->MLbase));

            			dml = &(DMLi1[j-2]);
            			decomp = dml->energy + separationModifier(j-i, E_MLstem(tt, S1[j-1], -1, P) + P->MLbase + P->MLclosing);
            			minState(cnode, decomp, ML_DECOMP_CLOSING_BP, dml->ileft, dml->jleft, FMLIJ, dml->iright, dml->jright, FMLIJ);
                      //decomp = MIN2(decomp, DMLi1[j-2] + separationModifier(j-i, E_MLstem(tt, S1[j-1], -1, P) + P->MLbase));
                      break;
          }
        ...
        if (dangle_model==3) {
          ...
          for (k = i+2+TURN; k < j-2-TURN; k++) {
            ...
            if (type_2) {
              decomp = traceC[indx[k]+i+1].energy+ separationModifier(j-i, P->stack[type][type_2] + 2*P->MLintern[1] + P->MLclosing) +traceFML[indx[j-1]+k+1].energy;
              minState(cnode, decomp, COAXIAL_STACK_CLOSING_BP, i+1, k, CIJ, k+1, j-1, FMLIJ);
            }
            type_2 = rtype[ptype[indx[j-1]+k+1]];
            if (type_2) {
              decomp = traceC[indx[j-1]+k+1].energy+ separationModifier(j-i, P->stack[type][type_2] + 2*P->MLintern[1] + P->MLclosing) +traceFML[indx[k]+i+1].energy;
              minState(cnode, decomp, COAXIAL_STACK_CLOSING_BP, i+1, k, FMLIJ, k+1, j-1, CIJ);
            }
          }
          /* no TermAU penalty if coax stack */
          //decomp += separationModifier(j-i, 2*P->MLintern[1] + P->MLclosing);
          //new_c = MIN2(new_c, decomp);
        }
        checkChildren(cnode, i, j);
      ...
      if(type){
        ...
        minState(fmlnode, new_fML, FREE_ENDS, i, j, CIJ, -1, -1, FMLIJ);
      }
      ...
      */
      switch(dangle_model){
        ...
        case 0:   //new_fML = MIN2(new_fML, fML[ij+1]+P->MLbase);
                  ...
                  minState(fmlnode, traceFML[ij+1].energy + P->MLbase, DANGLING_NUCLEOTIDES, i+1, j, FMLIJ, -1, -1, FMLIJ);
                  minState(fmlnode, traceFML[indx[j-1]+i].energy + P->MLbase, DANGLING_NUCLEOTIDES, i, j-1, FMLIJ, -1, -1, FMLIJ);
                  break;
        /* double dangles */
        ...
        default:  mm5 = ((i>1) || circular) ? S1[i] : -1;
                  ...
                  tt = ptype[ij+1];
                  if(tt) {
                	  minState(fmlnode, traceC[ij+1].energy + E_MLstem(tt, mm5, -1, P) + P->MLbase,
                			  DANGLING_NUCLEOTIDES, i+1, j, CIJ, -1, -1, FMLIJ);
                	  //new_fML = MIN2(new_fML, c[ij+1] + E_MLstem(tt, mm5, -1, P) + P->MLbase);
                  }
                  tt = ptype[indx[j-1]+i];
                  if(tt) {
                	  minState(fmlnode, traceC[indx[j-1]+i].energy + E_MLstem(tt, -1, mm3, P) + P->MLbase,
                			  DANGLING_NUCLEOTIDES, i, j-1, CIJ, -1, -1, FMLIJ);
                	  //new_fML = MIN2(new_fML, c[indx[j-1]+i] + E_MLstem(tt, -1, mm3, P) + P->MLbase);
                  }
                  tt = ptype[indx[j-1]+i+1];
                  if(tt) {
                	  minState(fmlnode, traceC[indx[j-1]+i+1].energy + E_MLstem(tt, mm5, mm3, P) + 2*P->MLbase,
                			  DANGLING_NUCLEOTIDES, i+1, j-1, CIJ, -1, -1, FMLIJ);
      ...
      if(dangle_model != 5){
        ...
        for (decomp = INF, k = i + 1 + TURN; k <= j - 2 - TURN; k++) {
          decomp = traceFML[indx[k]+i].energy + traceFML[indx[j]+k+1].energy;
          min_dml(&(DMLi[j]), decomp, i, k, k+1, j);
          minState(fmlnode, decomp, ML_DECOMP, i, k, FMLIJ, k+1, j, FMLIJ);
          //Fmi[k]+fML[indx[j]+k+1]);
        }
      ...
      if (dangle_model==3) {
        ...
        for (decomp = INF, k = i+1+TURN; k <= j-2-TURN; k++) {
          ...
          type_2 = rtype[type_2];
          if (type && type_2) {
            decomp = traceC[indx[k]+i].energy + traceC[indx[j]+k+1].energy + P->stack[type][type_2] + 2*P->MLintern[1];
            minState(fmlnode, decomp, COAXIAL_STACK, i, k, CIJ, k+1, j, CIJ);
          }
            //		MIN2(decomp,
      ...
      checkChildren(fmlnode, i, j);
      //fML[ij] =
      //Fmi[j] = fmlnode->energy;     /* substring energy */


     //fprintf(stderr, "%d\t%d\t%d\t%d\t%d\t%d\n", i, j, cnode->energy, cnode->s, fmlnode->energy, fmlnode->s);
    }

    {
      ...
      //}
		...
		dml_node *FF; /* rotate the auxilliary arrays */
		FF = DMLi2;
		DMLi2 = DMLi1;
		DMLi1 = DMLi;
		DMLi = FF;
		for (j = 1; j <= length; j++) {
			DMLi[j].energy = INF;
			DMLi[j].ileft = DMLi[j].jleft = DMLi[j].iright = DMLi[j].jright = -1;
		}
    }
  }
...
//              }
//  }
  return traceFML[indx[length]+1].energy;
  	  //f5[length];
}
